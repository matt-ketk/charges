...
def main():
    ...
    wire0 = Wire(
        np.array([-wireLength / 2, 0]),
        np.array([wireLength, 0]),
        wireRadius
    )

    latticeIons = []
    # protons
    xNum = 25 # columns of lattice protons
    yNum = 3 # rows of lattice protons
    for i in range(0, xNum):
        for j in range(0, yNum):
            latticeIons.append(LatticeIon(
                np.array([-12E-10 * i + 10E-9, 6E-10 * j - 6E-10]),
                Constants.COPPER_ION_RADIUS,
                Constants.E,
                Constants.COPPER_MASS
            ))
