...
def forces(coords, charges):
  ...
  distances = np.sum(distanceVectors ** 2, axis = 0) ** 0.5
  ...
  chargeProducts = np.array([np.matmul(chargesCol, np.array([charges])),] * 3)

  # find forces between each pair of particles (3D array)
  forcesSplit = -1 * Constants.K * distanceVectors * np.divide(chargeProducts, distances + 1E-12) # added 1E-12 to avoid divide by zero error 

  # find forces on each particle from by summing the forces from all the other particles from forcesSplit (2D array)
  forces = np.transpose(np.sum(forcesSplit, axis = 2))
...
def deltaPosition(dt, forcesList, velList, massList):
  ...
  massList = np.transpose(np.array([massList]))
...
def deltaVelocity(dt, forcesList, massList):
  ...
  accelerations = np.divide(forces, massList)
